...
public class PlayerInfoCommand extends NetworkCommand {
    ...
    private TextComponent replace(String replace, NetworkPlayer player){
        ...
        TextComponent component = GeneralUtil.replaceTextComponent(replace
                ...
                .replace("[points]",""+player.getHistory().getPoints())
                .replace("[chatPoints]",""+player.getHistory().getPoints(BanType.CHAT))
                .replace("[banPoints]",""+player.getHistory().getPoints(BanType.NETWORK))
                ...
                .replace("[isBanned]",""+(player.isBanned(BanType.NETWORK)?Messages.TRUE:Messages.FALSE))
                .replace("[isMuted]",""+(player.isBanned(BanType.CHAT)?Messages.TRUE:Messages.FALSE))
